...
def ious(axyxys, bxyxys):
    ...
    """
    ious = np.zeros((len(axyxys), len(bxyxys)), dtype=np.float32)
    if ious.size == 0:
        return ious

    axyxys = np.ascontiguousarray(axyxys, dtype=np.float32)
    bxyxys = np.ascontiguousarray(bxyxys, dtype=np.float32)
